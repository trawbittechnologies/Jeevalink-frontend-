...
export default function Requests() {
  ...
  useEffect(() => {
    ...
    // Polling mechanism for realtime updates
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-900 px-6 pt-6 pb-24 select-none">
          ...
          <div className="grid grid-cols-12 gap-6 items-start">
            ...
            <div className="col-span-7 space-y-4">
                ...
                <div className="flex gap-2 text-xs items-center">
                  {user && (
                    <button
                      ...
                      className={`px-2.5 py-1.5 rounded-xl font-bold transition-colors cursor-pointer text-xs border ${myRequestsOnly
                  ...
                  <select
                    ...
                    className="px-2.5 py-1.5 bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-xl font-bold text-slate-700 dark:text-zinc-300"
                  >
              ...
              {displayRequests.length === 0 ? (
                <div className="bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-3xl p-10 text-center text-slate-400">
            ...
            <div className="col-span-5 bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border /80 dark:border-zinc-800/80 rounded-3xl p-5 space-y-4">
              <div>
          ...
          <div>
            <div className="flex bg-slate-100 dark:bg-zinc-900 rounded-xl p-1 mb-5">
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1.5 ${activeTab === 'feed'
              ...
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1.5 ${activeTab === 'new'
            ...
            {activeTab === 'feed' ? (
              <div>
                <div className="flex gap-2 mb-4">
                  <select
                    value={filterBg}
                    onChange={(e) => setFilterBg(e.target.value)}
                    className="flex-1 px-3 py-2 bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-xl text-xs font-semibold text-slate-700 dark:text-zinc-300"
                  >
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-3xl p-5"
              >
                {renderRequestForm()}
              </motion.div>
            )}
      ...
      {selectedReq && (() => {
        ...
        let acceptedDonorIds = [];
        try {
          if (Array.isArray(selectedReq.accepted_donors)) {
            acceptedDonorIds = selectedReq.accepted_donors.map(String);
          } else if (typeof selectedReq.accepted_donors === 'string') {
            acceptedDonorIds = JSON.parse(selectedReq.accepted_donors || '[]').map(String);
          }
        } catch {
          // ignore
        }
        if (selectedReq.accepted_by) acceptedDonorIds.push(String(selectedReq.accepted_by));
        const hasAccepted = user && acceptedDonorIds.includes(String(user.id || user._id));
        ...
        return (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-6 text-center animate-fade-in">
            <div className="bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative">
              ...
              <div className="my-4 bg-slate-50 dark:bg-zinc-950 rounded-2xl p-4 border border-slate-100 dark:border-zinc-800/60 text-left space-y-2 text-xs">
                ...
                {(selectedReq.additionalNotes || selectedReq.additional_notes) && (
                  <div className="pt-2 border-t border-slate-200 dark:border-zinc-800">
                    ...
                    <p className="text-slate-700 dark:text-zinc-350 italic bg-white border-slate-200 shadow-sm dark:bg-zinc-900 p-2 rounded-lg border dark:border-zinc-800">{selectedReq.additionalNotes || selectedReq.additional_notes}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <a
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-colors ${callNum ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                    }`}
                >
                ...
                <a
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-colors ${waNumber ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                    }`}
                >
                ...
                <div className="mb-3 p-3 bg-red-50/60 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/40 text-left space-y-2">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    ...
                    <select
                      ...
                      className="px-2.5 py-1 bg-white border-slate-200 shadow-sm dark:bg-zinc-900 border dark:border-zinc-800 rounded-xl text-xs font-bold text-slate-900 dark:text-zinc-100 cursor-pointer"
                    >
                ...
                {!isOwner && !isPrivileged && (
                  hasAccepted ? (
                    <button
                      type="button"
                      onClick={async () => {
                        await unacceptRequest(selectedReq._id || selectedReq.id);
                        setSelectedReq(null);
                      }}
                      className="flex-2 flex items-center justify-center gap-1.5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 font-bold rounded-xl text-xs transition-colors cursor-pointer"
                    >
                      <XIcon className="w-3.5 h-3.5" /> Cancel Acceptance (Click to Cancel)
                    </button>
                  ) : (
                    <button
                      type="button"
                      onClick={() => acceptRequest(selectedReq._id || selectedReq.id)}
                      className="flex-2 flex items-center justify-center gap-1.5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold rounded-xl shadow-sm transition-colors cursor-pointer text-xs"
                    >
                      <Heart className="w-3.5 h-3.5 fill-white" /> Accept Request
                    </button>
                  )
                )}
              </div>
