...
import { Droplet, Clock, Building2, Phone, CheckCircle2, ShieldAlert, Flag, AlertTriangle, X, MapPin, Download, Edit3, Trash2, Heart } from 'lucide-react';
...
export default function RequestCard({ request, showActions = true }) {
  ...
  const { acceptRequest, unacceptRequest, fulfillRequest, fileComplaint, allUsers, deleteRequest, updateRequestStatus } = useAppStore();
  ...
  const acceptedList = (() => {
    try {
      let raw = request.acceptedDonors || request.accepted_donors;
      if (typeof raw === 'string') raw = JSON.parse(raw || '[]');
      if (Array.isArray(raw)) return raw.map(String);
      
      const accBy = request.acceptedBy || request.accepted_by || request.acceptedByUserId || request.accepted_by_user_id;
      if (accBy) {
        return [String(accBy)];
      }
      return [];
  ...
  const isAcceptedByMe = Boolean(
    currentUserIdStr && (
      ...
      String(request.acceptedByUserId || request.accepted_by_user_id) === currentUserIdStr ||
      String(request.acceptedBy || request.accepted_by) === currentUserIdStr
    )
  );
  ...
  const handleUnacceptRequest = async () => {
    setUnaccepting(true);
    await unacceptRequest(reqId);
  ...
  return (
    <>
      <div className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition-shadow text-left ${request.urgencyLevel === 'Immediate' ? 'border-red-300 ring-1 ring-red-200' : 'border-slate-100'
        }`}>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mb-3 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100 gap-2">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {isAcceptedByMe && (
                <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border flex items-center gap-1 ${myQueuePosition === 1
              ...
              {canManage ? (
                ...
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${request.status === 'Fulfilled'
            ...
            <div className="space-y-2 pt-2.5 border-t border-slate-100">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <a
                  ...
                  className={`flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-bold rounded-xl transition-colors ${whatsappNumber
              ...
              {/* Single morphing button for regular donors: Accept -> Cancel Acceptance */}
              {!isOwner && !isPrivileged && ['pending', 'waiting', 'accepted'].includes((request.status || '').toLowerCase()) && (
                isAcceptedByMe ? (
                  <button
                    type="button"
                    onClick={handleUnacceptRequest}
                    disabled={unaccepting}
                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 text-xs font-bold rounded-xl transition-all cursor-pointer shadow-xs disabled:opacity-50"
                  >
                    <X className="w-4 h-4" /> {unaccepting ? 'Canceling...' : 'Cancel Acceptance (Click to Cancel)'}
                  </button>
                  ...
                  <button
                    ...
                    disabled={accepting}
                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm shadow-emerald-500/20 cursor-pointer disabled:opacity-50"
                  >
                    <Heart className="w-4 h-4 fill-white" /> {accepting ? 'Processing...' : 'Accept Blood Request'}
                  </button>
                )
              )}
