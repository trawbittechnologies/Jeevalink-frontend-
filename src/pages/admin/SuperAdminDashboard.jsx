...
import {
  ShieldCheck, Plus, Key, RefreshCw, Eye, EyeOff, Edit3, Trash2, X, Building2,
  CheckCircle2, Users, UserCheck, Activity, BarChart3, TrendingUp,
  Search, Phone, Mail, Lock, LayoutDashboard
} from 'lucide-react';
  ...
  const handleCreateBlockAdmin = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/super-admin/block-admins', {
        ...
        block_admin_2_name: secondaryContactName,
        block_admin_2_mobile: secondaryContactMobile,
      ...
      if (res.data?.success) {
        setCreatedResult({
          type: 'success',
          msg: `Block Admin created successfully for ${blockName}! Password: ${res.data.data?.generated_password || 'Auto-generated'}`
        });
        setBlockName(''); 
      ...
      setCreatedResult({ type: 'error', msg: err.response?.data?.message || 'Network error while creating Block Admin.' });
    }
  };
